...
+ (float) pixelFromDegree: (float) degs {//console.log("pixelFromDegree");
    var mm = Math.tan(degs * Math.PI / 180.0) * 10.0 * [Settgs distanceInCM];
    return [self pixelFromMillimeter: mm];
}
+ (float) degreeFromPixel: (float) pixel {//console.log("Misc>pixelFromDegree");
    return 180.0 / Math.PI * Math.atan2([self millimeterFromPixel: pixel], [Settgs distanceInCM] * 10.0);
}


+ (float) millimeterFromPixel: (float) pixel {
    return pixel * [Settgs calBarLengthInMM] / [Settgs calBarLengthInPixel];
}
+ (float) pixelFromMillimeter: (float) inMM {//console.log("pixelFromMillimeter");
    return inMM * [Settgs calBarLengthInPixel] / [Settgs calBarLengthInMM];
}
...
+ (CPString) stringFromNumber: (float) num decimals: (int) decs localised: (BOOL) locd {//console.log("stringFromNumber");
    ...
    if (locd && ([Settgs decimalMarkChar] != ".")) {
...
static public function luminance2deviceGrey(luminance:Number):Number {
return Math.pow(luminance, 1.0/Settgs.gammaValue.n);
}
static public function deviceGrey2luminance(luminance:Number):Number {
return Math.pow(luminance, Settgs.gammaValue.n);
}
////////////////////////////////////// deviceGrey <—> RGB
static public function deviceGrey2RGB(theDeviceGrey:Number):uint {
var g:uint=limit(0, Math.round(255 * theDeviceGrey), 255);
return (g | g << 8 | g << 16);
}
...
static public function luminance2RGB(luminance:Number):uint {
var temp1:Number = luminance2deviceGrey(luminance);
return deviceGrey2RGB(temp1);
}
//////////////////////////////////////
