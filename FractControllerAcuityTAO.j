        ...
        case kStateDrawFore:
            ...
            if (!responseButtonsAdded) {
                for (let i = 0; i < (nAlternatives); i++) {
                    const button = [self buttonCenteredAtX: (i + 0.75) * 2 * size y: viewHeightHalf - 0.5 * size size: size title: "" keyEquivalent: [@"1234567890" characterAtIndex: i]];
                    [button setImage: [gAppController.taoController imageNumber: i]];
                    [button setImageScaling: CPImageScaleProportionallyDown];
                }
                [self buttonCenteredAtX: (10 + 0.75) * 2 * size y: viewHeightHalf - 0.5 * size size: size title: "Ø"];
            }
