 ...
 */
+ (BOOL) unittest {
    let success = YES, report = CPCarriageReturnCharacter + "AlternativesGenerator▸unittest:" + CPCarriageReturnCharacter;
    ...
    if (results4.some(v => ![0, 2, 4, 6].includes(v))) {
        report += "  ERROR: 4-alternative generator produced non-cardinal values: " + results4 + CPCarriageReturnCharacter; success = NO;
    }
    ...
    if (results2.some(v => ![0, 4].includes(v))) {
        report += "  ERROR: 2-alternative generator produced unexpected values: " + results2 + CPCarriageReturnCharacter; success = NO;
    }
    ...
    if (results4O.some(v => ![2, 4, 6, 8].includes(v))) {
         report += "  ERROR: 4-alternative oblique generator produced unexpected values: " + results4O + CPCarriageReturnCharacter; success = NO;
    }
    ...
    if (results10.some(v => v < 0 || v > 9)) {
         report += "  ERROR: 10-alternative generator out of range: " + results10 + CPCarriageReturnCharacter; success = NO;
    }

    if (success) {
        report += "  Sequence generation and range tests passed." + CPCarriageReturnCharacter;
    }
    console.info(report);
    return success;
}
