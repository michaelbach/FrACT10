...
- (void) applicationDidFinishLaunching: (CPNotification) aNotification { //console.info("AppController>…Launching");
    ...
    [self setupWindowEventListeners];
...
- (void) setupWindowEventListeners { //console.info("setupWindowEventListeners")
...
- (void) settingsDidChange: (CPNotification) aNotification { //console.info("settingsDidChange");
    ...
    [calibrationWarningField setStringValue: isNotCalibrated ? "(uncalibrated)" : ""];
}
...
- (void) setResultStringFieldTo: (CPString) s {
    ...
    while (size.width > maxWidth && fontSize > 8) {
        fontSize -= 1;  font = [CPFont systemFontOfSize: fontSize];
        size = [s sizeWithFont: font];
    }
    [resultStringField setFont: font];
}
