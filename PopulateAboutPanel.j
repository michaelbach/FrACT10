...
+ (void) populateHelpPanelView1: (WebView) v1 v2: (WebView) v2 v3: (WebView) v3 v4: (WebView) v4 {
    s = "<h4>General</h4> <ul><li><b>Make use of the tool tips</b> by hovering with the mouse over the pertinent interface element.</li> <li> Be sure to set the blue calibration bar (easiest: use plastic card) and the observation distance in Preferences correctly. Preferences are automatically saved.</li> <li> This is a forced choice test – even when your observers indicate that they cannot see a thing, encourage them to make their best guess; the software knows about the lucky guesses. Motivate them to react swiftly, lengthy squinting at the optotype is not helpful.</li> <li> Quality results requite knowledge in sensory testing. At least observe the Checklist (button below).</li> <li> The Manual (button below) should cover everything. For open questions contact me <a href='mailto:bach@uni-freiburg.de'>bach@uni-freiburg.de</a>.</li></ul>";
